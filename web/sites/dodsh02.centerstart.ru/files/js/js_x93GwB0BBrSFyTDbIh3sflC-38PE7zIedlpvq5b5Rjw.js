...
window.addEventListener('DOMContentLoaded', (event) => {
  setTimeout(() => {
    const bvi = document.querySelectorAll('.bvi-panel-close')
  console.log(bvi.length)
  console.log(!!bvi)
  if (bvi.length) {
    document.querySelectorAll('.news-teaser__link')
    .forEach(el => el.classList.toggle('link-bvi'))
  }
 }, 500)

  document.querySelector('.bvi-open').addEventListener('click', e => {
      document.querySelectorAll('.news-teaser__link')
      .forEach(el => el.classList.toggle('link-bvi'))
  })
  
  const theId = "bvi-panel-close";
  document.querySelector(`#${CSS.escape(theId)}`).addEventListener('click', e => {
    document.querySelectorAll('.news-teaser__link')
    .forEach(el => el.classList.toggle('link-bvi'))
  })
});
