...
window.addEventListener('DOMContentLoaded', (event) => {
  const title = document.querySelectorAll('.field.field--name-title.field--type-string.field--label-hidden');
console.log(title); 

if (title.length > 0) {
  title.forEach(element => element.classList.remove('field--label-hidden'));
}
});


// $('.news-teaser__title').forEach(element => element.children().removeClass());
